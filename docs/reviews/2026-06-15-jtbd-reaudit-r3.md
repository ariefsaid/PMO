...
- **G3c — CRM hub completion: ✅ DONE** (`10c120e`, 3023 unit / typecheck 0 / lint 0). CD-1/CT-1 editable+deletable activity (RLS `crm_activities_write` is `for all`/MASTER_DATA org-scoped — no migration; added `updateActivity`/`deleteActivity` via repo seam + `contactActivity` edit/delete policy — **new delete surface → security re-check in final battery**); CD-3 type-independent related procurement; CD-4 cold-start Activity prompt.
- **G3d — Mobile overflow + consistency + dead-code: ✅ DONE** (`e2dbe38`, 3046 unit / typecheck 0 / lint 0 / build ok). PageHeader/BvACard @390 (+`PageHeader.mobile.test`, ProgressBar `widthless` variant — global `cn` untouched); `roleVariant`/`budgetVersionVariant`→registry (Finance role pill `won`→`neutral`); noun fix; deleted 3 dead components (ProjectPipelineStepper/ProjectStatusBadge/TimesheetStatusBadge, zero importers confirmed) + stale ContactDrawer comment; 2 guard tests (drawer-activation, status-registry); xlsx export catch→toast.

**fix-wave-3 BUILD COMPLETE** (G3a `e1e2b0d` · G3b `9774e5c` · G3c `10c120e` · G3d `e2dbe38`). Gate: typecheck 0 · lint 0 · build ok · **3046 unit** · **496 pgTAP** (FE-only wave, no new migration). **▶ NEXT: convergence battery** — security re-check on G3c's new activity update/delete surface + **final fresh re-audit (both oracles)**; converged ⇒ PR when it returns 0 Crit / 0 Imp.
