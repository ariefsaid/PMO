...
| **Migration numbering** | Highest existing migration is **`0166_attestation_releases_the_held_mirror.sql`**; highest pgTAP test is **`0159_attestation_admits_the_reopen.test.sql`**. ⚑⚑ **RE-ALLOCATED 2026-07-27 — the warning below came true.** PR #396 landed migration **`0168`** and pgTAP **`0161`** on `dev` (`save_timesheet_week_entry_date_bounds`) — **exactly the two numbers this plan had reserved for PR-2**, while leaving PR-1's `0167`/`0160` free. **Binding current allocation: PR-1 → migration `0167`, test `0160` (unchanged, still free). PR-2 → migrations `0169`, `0170`, tests `0162`, `0163` (shifted up one from the task text below). PR-4 → migration `0171`, test `0164` (unchanged, still free).** The Slice B task bodies below still say `0168`/`0169` and `0161`/`0162` — **rename to the shifted numbers when you write them**; the embedded UUID prefixes (`01610000-…`) are cosmetic and need not follow. Re-check again before writing: `git ls-tree --name-only origin/dev supabase/migrations/ | sort | tail -3`. `npm run check:migrations` catches a duplicate prefix at verify time, but only *after* you have written the file. ⚑ A parallel branch may also allocate — before writing, re-run `ls supabase/migrations | tail -3` and `ls supabase/tests | sort | tail -3` and shift the whole block up if taken. `npm run check:migrations` catches collisions at verify time. |
...
| **PR-2** | **B — Money & concurrency** | §4.6 (FR-HRD-040/041/043) | `supabase/migrations/0169`–`0170`, `supabase/tests/0162`–`0163` *(shifted 2026-07-27, see §0)*, `.github/workflows/spike-rls.yml`, `spike/agent-native-rls/.gitignore` | — |
