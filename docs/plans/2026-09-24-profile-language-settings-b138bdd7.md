...
3. Only after that write resolves does it call a new `refreshCurrentUser()` Auth-context method. The method reuses the provider’s existing `loadProfile` fetch and atomically replaces `currentUser` only when a profile is returned; it rejects on a refresh failure and preserves the previously usable profile rather than converting a successfully authenticated session into a false success state.
...
- Add a test titled `refreshCurrentUser rejects and preserves the current profile when the refresh read fails` that switches the second query result to a generic error; assert no success signal, the original profile remains rendered, and the returned promise is rejected.
...
**Edit** `pmo-portal/src/auth/AuthContext.ts` and `pmo-portal/src/auth/AuthProvider.tsx`:

- Add `refreshCurrentUser: () => Promise<void>` to `AuthContextValue`.
- In `AuthProvider`, create a memoized refresh callback that reads the current session user id, calls existing `loadProfile`, and, on success, sets `currentUser` plus clears the two profile-error fields. If there is no current session user, reject rather than silently reporting a refresh that did not occur. If `loadProfile` returns an error, throw an `Error` carrying its message **without clearing the existing `currentUser`**; the settings page will surface that failure and must not claim success.
- Include the callback in the context value and `useMemo` dependencies. Do not alter sign-in, sign-out, auth-event, impersonation, or the existing initial-load behavior.
...
2. select Organization default and save, then assert `setMyLocalePreferences` is called with the authenticated user id and `{ locale: null, numberLocale: <unchanged profile value>, timezone: <unchanged profile value> }`, then `refreshCurrentUser` is called before the polite success status appears;
...
4. reject the DAL mutation and separately reject `refreshCurrentUser`, asserting an assertive error is shown and no success status appears in either case; and
...
- Call the DAL only for `currentUser.id` and copy `currentUser.number_locale`/`currentUser.timezone` unchanged. Await `refreshCurrentUser()` after a successful DAL write; show success only when both awaits resolve. On either rejection, render an error state and keep the chosen value retryable. Do not add controls or writes for number locale/timezone.
