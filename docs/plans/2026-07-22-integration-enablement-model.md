...
|---|---|---|
| AC-EAS-001 | Unit | `supabase/functions/_shared/externalExecutionEnabled.test.ts` |
| AC-EAS-002 | Unit | `supabase/functions/_shared/perOrgSecret.test.ts` |
| AC-EAS-003 | pgTAP | `supabase/tests/integration_enablement_model.test.sql` |
| AC-EAS-004 | Curated E2E | `pmo-portal/e2e/AC-EAS-004-connect-atomic-success.spec.ts` |
| AC-EAS-005 | Unit | `supabase/functions/external-connect/connect.test.ts` |
| AC-EAS-006 | pgTAP | `supabase/tests/integration_enablement_model.test.sql` |
| AC-EAS-007 | Curated E2E | `pmo-portal/e2e/AC-EAS-007-kill-switch-recovery.spec.ts` |
| AC-EAS-008 | pgTAP | `supabase/tests/integration_enablement_model.test.sql` |

## Implementation tasks (TDD-first, 2–5 minutes each)

### Shared switch and resolver

1. **Write the failing switch tests** *(AC-EAS-001, NFR-EAS-001).* Create `supabase/functions/_shared/externalExecutionEnabled.test.ts` with cases for absent, `true`, `false`, `1`, and `yes`; assert only exact `false` disables. Verify: `cd supabase/functions/_shared && deno test externalExecutionEnabled.test.ts`.
...
5. **Write execution-path switch tests** *(AC-EAS-001, AC-EAS-009).* Add cases to the existing Deno tests for `adapter-dispatch`, `clickup-sweep`, `clickup-webhook-worker`, `erpnext-onboard`, `erpnext-sweep`, and `erpnext-webhook` proving false stops external calls and restoring the switch reuses the existing binding. Verify: `scripts/with-test-lock.sh bash -c 'cd supabase/functions && deno test adapter-dispatch clickup-sweep clickup-webhook-worker erpnext-onboard erpnext-sweep erpnext-webhook'`.
...
7. **Write failing readiness tests** *(AC-EAS-003, AC-EAS-004).* Extend `supabase/functions/external-connect/connect.test.ts` with: switch false; Vault miss; adapter readiness failure; and successful readiness. Assert ownership RPC is zero calls on each failure and occurs only after readiness on success. Verify: `cd supabase/functions/external-connect && deno test connect.test.ts`.
...
10. **Write client failure tests** *(AC-EAS-005).* Add cases to `pmo-portal/src/components/integrations/IntegrationsView.test.tsx` for disabled, invalid-token, and sync-not-ready responses; assert an actionable error, no Active badge, and no credential text in rendered output. Verify: `cd pmo-portal && npx vitest run src/components/integrations/IntegrationsView.test.tsx`.
...
14. **Add the atomic success journey** *(AC-EAS-004).* Create `pmo-portal/e2e/AC-EAS-004-connect-atomic-success.spec.ts`; as an org-admin, submit a valid ClickUp credential, assert Active only after the finalize response, and assert a sync request uses the org binding. Mock external HTTP only at the boundary. Verify: `cd pmo-portal && npx playwright test e2e/AC-EAS-004-connect-atomic-success.spec.ts`.
15. **Add kill-switch restoration journey** *(AC-EAS-007).* Create `pmo-portal/e2e/AC-EAS-007-kill-switch-recovery.spec.ts`; start with an active binding, stop execution, restore execution, and assert sync resumes without a reconnect or credential-entry step. Verify: `cd pmo-portal && npx playwright test e2e/AC-EAS-007-kill-switch-recovery.spec.ts`.
