...
3. Add an “Available powers” table using the five required fields for: feature entitlement toggles (`Administration → Features` / `operator_toggle_feature`); org-targeted invites (`admin-invite-user`); credit grants; operator directory and aggregate usage/run-stat reads; cross-org user-status updates; and `operator_set_domain_ownership`.
...
**Verify:**
```bash
rg -n "Does|Invoke|Who may|Audited as|Verify/stop|operator_grant_credits|operator_list_orgs|operator_usage_summary|operator_agent_run_stats|admin_set_user_status|operator_set_domain_ownership|admin-invite-user|platform_operators" docs/operator-runbook.md
```
...
| 2 | Every current operator power; discover/report powers omitted from the issue list; DD-OPS-1 boundary rule and revisit trigger |
