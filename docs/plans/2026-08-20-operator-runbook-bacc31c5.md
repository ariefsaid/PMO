...
|---|---|---|
| Feature entitlement toggles | `/administration` → **Features** UI invokes `operator_toggle_feature(p_org_id, p_key, p_enabled)`; only the platform Operator can change a gatable key. | **Not currently audited.** |
| Org-targeted invitation | `admin-invite-user` with `email`, `role`, `p_org_id`; source also shows the Operator can reach this from `/administration` → **Users** → **Invite user** and select a target organization, so correct the issue's now-stale “API-only” statement rather than repeating it. Org-Admins are pinned to their own organization. | **Not currently audited** on the successful invitation path. |
| Cross-org member offboard/re-enable | `admin_set_user_status(p_profile_id, p_status, p_org_id)` from the Users UI; both an in-org Admin and an Operator may use it, with the sole-Admin/self lockout protections. | **Not currently audited.** |
| Credit grant | `/administration` → **Credits** → **Grant credits**, enforcing `operator_grant_credits(p_org_id, p_amount, p_note)`; Operator only. | `credits.grant` in `audit_events`. |
| Organization directory and aggregate usage/read statistics | `/administration` → **Usage** (Operator-only); `operator_list_orgs()`, `operator_usage_summary(p_org_id)` and `operator_agent_run_stats(p_org_id)` provide the switcher and aggregate views. | Read-only access is **not currently audited**. |
...
Add a concise **Additional source findings** section so the page truly reports code discoveries rather than silently limiting itself to the issue's initial list:

- `recover_external_connect_trap(...)` is service-side recovery for a trapped ClickUp connection, requires a platform-Operator actor plus the controlled recovery procedure, and writes `integration.trap_recovery`.
- The external integration connect/list/link/unlink/disconnect/set-company functions recognize an Operator as an alternate authorization path, but are also routine integration actions available to their in-org Admin (and, where applicable, PM). Group these as **not operator-exclusive** and link to their existing in-app integration surfaces rather than duplicating credential-bearing procedures. State their individual success-path audit actions only after checking the shipped function; where a function has none, write “not currently audited.”
- Adapter onboarding and historical import are privileged provisioning/service procedures, not browser-Operator RPCs. Give a neutral stub pointing to their existing named procedure/guides; do not reproduce credentials, secrets, request bodies, hosts, or service-side invocation details. This reports the discovery without publishing a dangerous operator recipe.
...
4. Describe the invite UI discovery truthfully: it is reachable from the Users invite flow for an Operator in addition to its Edge Function interface; preserve the non-Operator own-org restriction.
5. Add an “Additional source findings” section covering trapped-integration recovery, Operator-authorized but non-exclusive integration actions, and privileged onboarding/import procedures. Use the required neutral stubs when documenting their details would expose credential-shaped material.
6. Do not copy any SQL, host, environment variable, secret coordinate, account details, org identifier, or personal data from existing environment documentation.

**TDD:** Not applicable — this documents existing behavior and adds no executable product behavior.

**Verify:**
```bash
rg -n "Does|Invoke|Who may|Audited today|operator_toggle_feature|admin-invite-user|admin_set_user_status|operator_grant_credits|operator_list_orgs|operator_usage_summary|operator_agent_run_stats|operator_set_domain_ownership|platform_operators|recover_external_connect_trap|credits\.grant|integration\.trap_recovery|not currently audited" docs/operator-runbook.md
```
...
|---|---|
| 1 | Every shipped Operator power; invocation, authority, and current audit truth; report source-discovered powers; DD-OPS-1 boundary/revisit decision |
