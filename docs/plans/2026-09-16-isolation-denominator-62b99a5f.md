...
1. It invokes bare `psql` with `-X -A -t -v ON_ERROR_STOP=1 -d <DATABASE_URL>` (defaulting to `postgresql://postgres:postgres@127.0.0.1:54322/postgres`) and parses one JSON object per row for public tables. The query is limited to `pg_class.relkind IN ('r', 'p')` in `public`; `has_org` is an `EXISTS` check for a live, non-dropped `org_id` attribute; the primary-key attribute is obtained from the primary index in attribute order via `ORDER BY a.attnum LIMIT 1`. Discovery rejects a table with NO primary key (so no `pk` the current probe could never target); a **composite** (multi-column) primary key is emitted using its first key column — exactly what the probe's single-column `$pk=eq.<id>` selector already targets, and it keeps all 83 tables in the denominator (the issue requires every public table enumerated).
...
3. Use these exact SQL selection rules (ordering is part of the query):
   ...
   ```
   Validate that every discovered table has a non-empty primary-key attribute (`pk`) before it can be emitted; fail with the table name and a request to make the probe's key strategy explicit if not. A composite primary key is not a failure: its first key column becomes the table's `pk` (see the Catalog-contract note), preserving all 83 tables in the denominator.
