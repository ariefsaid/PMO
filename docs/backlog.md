...
### ⚑⚑ ACTIVE HANDOFF (2026-07-07, ~17:00 — Director hit 5h quota mid-Criticals; GLM lanes) ⚑⚑
**3 Criticals are IN-FLIGHT on separate worktrees off `9dabd03`, NOT merged.** Substrate: glm-5.2 (opus alt) + glm-4.7 (sonnet alt), one parallel dispatch per model. All money/security-path → Director security-reviews every diff. Verify pgTAP SERIALLY in the main session under `scripts/with-db-lock.sh` (the lock does NOT cover the pi agents — a live pi run on the shared Supabase Docker stack collides with `supabase db reset`; serialize by hand: don't reset while a pi agent is alive → `pgrep -fl "pi --provider"`).

- **#16 automation double-fire — ✅ DONE, GREEN, committed (NOT merged).** Branch `fix/automation-double-fire` (worktree `PMO-worktrees/automation-claim`), 3 commits (tip `af7dd24`). Mig `0078_automation_fire_claim.sql` (per-`(automation_id,event_id)` PK claim table, service-role-only, no policy) + dispatcher claim-then-fire in `agent-dispatch/dispatcher.ts` (`claimTriggerFire`) + pgTAP `0135` (AC-AUTOFIRE-001..007). Full suite **1135 PASS**, `deno check` green. **Ready to merge to dev.** (Director fixed glm-4.7's hallucinated `.on().ignore()` supabase-js API + `has_table`/`trigger_on` pgTAP bugs.)
- **#17 audit_events + log_audit() — CODE DONE + Director-fixed, committed, NOT re-verified green.** Branch `harden/audit-events` (worktree `PMO-worktrees/audit-events`), tip `221aeac` (glm-5.2 `1d5ec7a` + Director fix). Mig `0076_audit_events.sql` (append-only table, FORCE RLS, one SELECT policy own-org Admin/Operator, `log_audit()` postgres-owned SECURITY DEFINER sole writer, wired into `operator_grant_credits`/`set_project_contract_value`/`transition_document_status` + companies/projects AFTER-DELETE triggers) + RED pgTAP `0133` (28 asserts). **Director caught 3 defects (see commit `221aeac`): (1) transition_document_status was copied from STALE 0017 → dropped 0025's auto-Supersede logic → `0066_document_superseded` regressed; re-based on 0025. (2) hardened append-only to privilege-denied (no U/D grant). (3) fixed invalid-hex fixture UUID.** ⚠️ **NEXT: clean `supabase db reset && supabase test db` on an UNCONTENDED stack** (last run collided with the live reserve_credits pi → false mass-fail); expect 0133 + 0066 both green, then merge.
- **#15 reserve_credits race — DB PRIMITIVE + PROOF committed; call-site wiring ESCALATED/deferred (race NOT closed-in-practice yet).** Branch `fix/reserve-credits-race` (worktree `PMO-worktrees/reserve-credits`), tip `1ede96b` (glm-5.2, self-committed; only stray `deno.lock`s left untracked). Mig `0077_reserve_credits.sql`: `credit_reservations` hold-ledger (RLS force, no policy, `(org_id) where released_at is null` index) + `reserve_credits(org,amount,run_id)` under `pg_advisory_xact_lock(hashtext('credits:'||org))` (available = credits − usage − unreleased holds; insufficient→23514) + idempotent `release_credits(run_id)`. Guard `creditRateGuard.ts` gains `check(orgId, runId?)` — reserves when a runId is passed, else the read-only `org_credit_balance` path (back-compat). pgTAP `0134` (AC-RESERVE-001..006). `RESERVE_UNIT=0.01`. gl-agent ran typecheck/lint/build/unit(818) + an isolated-postgres empirical race proof (5 trials, exactly one 80-of-100 reserve won). **⚠️ NOT yet Director-security-reviewed + pgTAP not run on the shared stack. ⚠️ DORMANT: all 4 `check()` call-sites pass no runId → reserve/release are NEVER invoked → zero holds; combined with `AGENT_CREDITS_ENFORCED` OFF in prod, the race is neither closed-in-practice nor exploitable today.** **The actual fix needs the coupled pair the agent deliberately did NOT wire (would leak holds→org-lockout if half-done): thread `run_id` through the 3 agent-chat `check()` sites + a `release_credits` after each `recordUsage`/`insertUsageRow`; decide compose-view's missing run_id (it passes null — release by the returned reservation id, or a TTL reaper for orphaned holds).** So audit Reliability-Critical reads "primitive landed, race still open, wiring deferred".
- **Merge order:** land #16 first (green), then #17 (after clean re-verify + fold in the audit-gap below), then #15 (after Director review + serial pgTAP — mergeable as the dormant primitive even before the call-site wiring, since it changes no live behavior), then **#14 supply-chain LAST** so dev doesn't churn. Each is PR→dev (verify-only fast lane). Then batch to main.

**⚑ Second-auditor reconciliation (2026-07-07) — gaps NOT covered by the in-flight work, fold in before closing:**
- **[fold into #17] Agent SoD-refusal / PERMISSION_DENIED is not durably audited (Obs-High).** `0076` wires `log_audit` into 3 RPCs + 2 delete triggers but NOT the agent-chat authz-refusal path (`agent-chat/handler.ts` ~839/1383) — a prompt-injection/misconfig escalation attempt vanishes with the SSE stream. Same mechanism/migration as #17 → add one `log_audit`(action `agent.permission_denied`) call there while #17 is open.
- **[rides with #16 / agent-dispatch] `advanceWatermark` has no error handling → trigger re-fire (Rel-Med)** — a DIFFERENT double-fire mechanism than #16's claim; fix in the same file pass. Also **`select_trigger_events` trusts caller `p_filters[].org_id` (Adv-Med)** — mitigated (service_role-only + JS re-assert) but constrain org in the RPC body.
- **[new backlog Highs] e2e blindspots (Test-High):** 4 hard-skipped Incidents-quarantine e2e (nothing flags the un-skip on flag-flip) + 2 silent-skip when `SERVICE_ROLE_KEY` absent (report green with those ACs never run).
- **[new backlog Meds/Lows]** telegram-notify send-ok+stamp-fail → dup alerts (no err check `index.ts:86`) · `notifyOwner` swallows errors untraced · health endpoint checks zero deps (green ≠ healthy DB/auth) · `enforce_automation_owner_cap` racy count-then-insert (SHARE ROW EXCLUSIVE fix pattern already at `0065:69`) · `set_project_contract_value` still accepts negative/unbounded (overlaps "money CHECK(>=0)"; #17 now logs but doesn't reject) · manifest↔lockfile uuid-override mismatch (confirm #14's lockfile regen records it) · `spike-rls.yml` uses `npm install` (not ci) with a service-role key in scope (pin+ci or delete the spike) · 3 missing runbooks (prod-deploy, secret-rotation, agent-LLM-outage — doc conversions from `environments.md`, not new work).
...
- **Remaining Highs/Meds (not started, from the earlier audit):** feature-flag server-enforcement (conjoin `org_has_feature` into `*_write` RLS) · orphaned-Auth-user compensation (`admin-invite-user`) · agent-persistence error handling (stuck `running`) · interactive-create idempotency · `error_events` completeness (2 fns + FE) + retention · S-curve today-position deterministic test · money `CHECK (>=0)` · PostHog consent-gate · agent-chat rate-limit.
