...
## ▶ Current state (2026-06-17) — MOBILE-BLEED GATE + POSTHOG fixed (merged); S-CURVE actual-line in CI

> **RESUME ENTRY POINT (model-agnostic).** `main` is ahead of prod again. **Prod is PARKED** at `production`=`a1e5115` / Cloud DB **migration 0033** (the 2026-06-16 promote) — **owner directive: ship new work to `main` ONLY, no prod promote** (the eventual promote will carry mobile + PostHog + S-curve + migration 0034 together, on the owner's call).
>
> **⭐ IMMEDIATE NEXT ACTION — fix PR #136 (S-curve actual line) verify-red, then merge to main.** Branch `feat/scurve-actual-line` (PR [#136]). **`integration` is GREEN** (migration 0034 + pgTAP + the new e2e `AC-SCA-014` all pass on CI). **`verify` is RED** on two regressions the subagents' *targeted* test runs missed (full-suite-only): **(a)** `pages/project-detail/__tests__/ProjectDetail.defaultTab.test.tsx` — 7/10 fail because `ProjectSCurve` now calls `useTasks(projectId)` and tests rendering ProjectDetail/Overview don't mock it (add a `vi.mock('@/src/hooks/useTasks', …)` returning `{data: []}` — also check any other suite that renders ProjectSCurve/ProjectDetail); **(b)** a `tsc --noEmit` error (likely the new `e2e/AC-SCA-014-actual-line-moves.spec.ts` or a type in the changed files — run `cd pmo-portal && npm run typecheck` on the branch to get it). Fix both, re-push, CI green → **merge to main** (Director-approved; no prod promote).
>
> **Shipped to main this session (post-Gantt):** **PR #135 MERGED** — (1) **mobile horizontal-bleed killed app-wide @390/360** (header→avatar menu, procurement row+filter toolbar, overview cards, timesheets select, DataTable inline-truncate cells) + the **measuring gate `e2e/AC-MOBILE-OVERFLOW-001`** (every route×{390,360} asserts no element right-edge > viewport — the deterministic L1 gate that the 4-lens reviews structurally couldn't be; now graduated in `qa-portfolio.md`); (2) **PostHog actually captures now** — root cause was our `property_denylist` stripping PostHog's own `token` field (posthog-js#3438) → tokenless `/e/` → 401; fix excludes `token` from the SDK denylist (`POSTHOG_PROPERTY_DENYLIST`) + a valid-`phc_`-key guard. **PR #134** (earlier) = the prod-promote ops docs + `scripts/db-seed-prod.sh`.
>
> **Executor switch (owner directive):** role work now runs on **Claude Task subagents, NOT pi** ("use subagents here instead of pi for now"). The S-curve issue ran the FULL per-issue loop on subagents and it worked well: feature-forge(spec)→eng-planner(plan+ADR)→implementer(migration+builder)→ui-implementer(chart+axis)→spec/quality/security-auditor(3-reviewer gate)→qa-acceptance(e2e). Background dispatch + `run_in_background:true` + auto-reinvoke = context economy. **Director still verifies every claim + does the rendered visual pass** (which caught 2 real bugs unit tests missed — see #136 below).
>
...
0. **⭐ FIX PR #136 verify-red → merge to main** (full detail in the RESUME ENTRY above): mock `useTasks` in `ProjectDetail.defaultTab.test.tsx` (ProjectSCurve now reads it) + fix the `tsc` error; integration is already green; then merge to main. **No prod promote.**
1. **ROW-CLICKABLE issue — QUEUED, intake done (owner-approved 2026-06-17).** Make the **whole row/card clickable**, not just chevron/title (better hit-target/flow, the normal convention). **Two behaviours by surface type:** *navigation lists* (procurement list, projects, tasks, companies, contacts, kanban/board cards) → **row-click opens the detail page**; *the /approvals **action queue*** (procurement + timesheet rows) → **row-click EXPANDS the in-place decision panel** (Approve/Reject + budget impact), NOT navigate — preserves the deliberate IF-A "decide without leaving the queue" flow. Nested controls (status select, ⋯ menu, Approve/Reject, links) must `stopPropagation`; keep keyboard focus. Apply **consistently** across the app. Run the full loop (right-sized — it's UI, mostly the DataTable row + ProcurementListRow + ProcurementApprovalRow). Owner is actively using the app and asked for this.
2. **PROD PROMOTE (parked, owner-gated)** — `main` → `production` + push migration **0034** to Cloud + reseed (the demo S-curve needs the seed.sql backfill block). Owner: **not yet — ship to main first.** Fold before promote: the 2 LOW pgTAP debt on mig 0028; 1 dependabot-high (esbuild, dismissed). Prod currently `a1e5115`/0033.
3. **Vendoring:** date-fns ✅ #130 · TanStack Table ✅ DEFER #131. Closed for now.
4. **Minor doc residuals + deferred review-nits** (non-blocking): a `tsToIso` helper in `sCurve.ts`; DRY the identical migration↔seed `completed_at` backfill expression; breakpoint-doc 768-vs-640.
