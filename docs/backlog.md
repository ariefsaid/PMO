...
- **▶ fix-wave-3 IN PROGRESS (plan-of-record in the r3 doc):**
  ...
  - **▶ G3b NEXT:** shared `CompanyNameLink`+`ContactNameLink` + apply across the dead-display sites (E-1: D-1/PL-1/PL-2/PRD-1/CD-2/AD-1/D-2) + `ApprovalRowShell` unification (consistency B — the owner's chevron complaint).
  - **G3c:** CRM editable/deletable activity (CD-1/CT-1) + type-independent related procurement (CD-3) + cold-start empty (CD-4).
  - **G3d:** mobile overflow (PageHeader/BvACard @390 + ProgressBar cn) + ROLE_PILL/VERSION_PILL→registry + noun fix + dead-code delete + guard tests + xlsx-export catch.
  - Then **central gate → final fresh re-audit → PR** (converged when re-audit returns 0 Crit / 0 Imp).
