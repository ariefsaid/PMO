...
### Next
- **✅ #14 supply-chain/CI MERGED** (PR #260, `ffbef40`) — deno.lock pins + `--frozen` gate + 12 Actions SHA-pinned + new pgTAP-on-PR→dev job (first run green, 2m12s). Merge onto current dev was conflict-free; `deno cache --frozen` confirmed the locks match post-merge edge-fn code. **All audit Criticals + top gaps + supply-chain/CI are now on `dev`.**
- **⏭ Promote `dev`→`main`** — the remaining step (Director-gated within the signed audit-remediation scope). Runs the `integration` lane (all pgTAP + e2e + visual) that PR→dev skips — the authoritative gate. Then `main`→`production` is a SEPARATE, owner-instructed-per-instance promote (never autonomous). Migrations to push to prod Cloud on that occasion: `0076–0079` (prod was ~`0060`).
