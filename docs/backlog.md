...
**⚑ Owner decision — RESOLVED (2026-06-09): Wave 3 = CORRECTNESS-FIRST** (silent-data/integrity bugs + lifecycle rework dead-ends + procurement authz gaps), **mobile = Wave 4** (near-term users are desktop). Build order within Wave 3: ✅ Part 1 = wrong-data bugs (F3/F4/F1/F2, PR #39 merged) → then **Part 2** = resilience (F5/F6/F7/F8 + G2 + D10) → **Part 3** = lifecycle dead-ends (B1/B2/B3+A8) → **Part 4** = procurement authz (A9 RPC over-grant + A10 + AC-W2-RBAC-011-RLS) **+ the new-audit correctness/RBAC items N1–N4 (see below)**.
...
- **NEW → OWNER-DECISION (touch prior deliberate decisions / sizeable scope):** **O1** timesheet forces an **intermediate Save before Submit** (3 post-entry clicks; the deliberate Wave-1 SP-2 co-location — Harvest/Toggl is 1-click; let Submit save valid dirty rows first?); **O2** procurement **create is split** modal→detail and persists a Draft before authoring is complete (cohesive composer vs two-step?); **O3** **Mark-Vendor-Invoiced is split from invoice capture** (state set before the invoice evidence is entered — intentional intermediate state, or co-locate?); **O4** **inline pipeline stage change** (drag/drop or stage dropdown from the board — sizeable build vs the current open-detail-to-advance). Also noted: stage-advance-opens-full-detail is the ADR-0021 tradeoff (awareness, not a bug).
