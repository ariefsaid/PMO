...
`review/OUTSTANDING.md` was triaged on the Wave-1 branch (before Wave 2 + the unified detail page). **Closed since:** Theme A RBAC view-gates **A1–A7** (Wave 2 Part A) + **A11** owner decisions resolved; Theme D IxD-Wave-2 **D1/D2/D3/D5/D14** + the hover-gating half of **D11/D12** + **G1** (Wave 2 Part B); Theme E nav cleanup **E1–E5** (Wave 2 B-5..B-10); **F9** dead bell/Export (Wave 2); the My-Tasks pill (Wave-2 follow-up); and **ADR-0021** unified detail page. What REMAINS, reprioritized:

**▶ Wave 3 (RECOMMENDED) — Correctness, integrity & lifecycle (cheap, high-trust, well-scoped; matches the owner's usability-first bar).**
- **Silent-failure / data-integrity bugs (Theme F):** ✅ **F3/F4** numeric→0 silent coercion + **F1/F2** stale fk-options pickers — **DONE, PR #39** (`main`@21c58b2; shared `parseMoneyInput` for validate==persist). Remaining: **F8** create modals submit with blank required fields (no disable/first-error focus) — shared `EntityFormModal` readiness; **F5** timesheet delete optimistic, no rollback; **F6** budget mutations drop DB error codes; **F7** unknown URL → silent dashboard (add 404).
- **Lifecycle rework dead-ends (Theme B):** **B1** timesheet Rejected→Draft (told "correct & resubmit", no control); **B2** document Rejected→Draft/Closed; **B3** Admin break-glass procurement moves (+ **A8** Admin edit Draft/Rejected PR header).
- **Procurement role/authz gaps (Theme A residual):** **A9** backend over-grant — `create_procurement_receipt` lets Finance create the GR (business-rule gap, needs RPC + pgTAP, security-auditor); **A10** Engineer requester can Confirm Receipt but can't capture the GR header (action ≠ record); **AC-W2-RBAC-011-RLS** Finance own-Draft timesheet server hole.
- **Pulled-forward quick correctness:** **G2** PM-dashboard "Project Status" never handles isPending/isError; **D10** a $0 / no-line-item PR can be quoted/approved/ordered ("Ready to advance" on an empty Draft).

**▶ Wave 4 — Mobile responsiveness (Theme C; the owner-flagged "big unanimous one").** A dedicated pass: **C1** mobile record-card/reflow for every data table (clips at 375px) + the timesheet grid; **C2** project-detail header reflow (now carries the unified tabs + deal banner — bigger surface post-ADR-0021); **C3** shell chrome (title truncates, ⌘K ~2px, breadcrumb collision → overflow); **C4** procurement stepper/summary clip; **C5** sales kanban horizontal overflow; **C6** `Button` `.touch-target` ≥44px. Largest effort; lower urgency IF near-term users are desktop (owner to confirm — see fork below).

**▶ Wave 5 — Detail-page & dashboard IxD polish (Theme D residual).** **D15** de-emphasize finance header chrome for non-PM/Finance on the (now-unified) project page — an Engineer on Tasks scrolls past finance data (MORE relevant post-ADR-0021); **D6** surface SoD "another approver needed" on the Draft up front; **D7** two blue primaries at Approved; **D8** Cancel-request card above the primary CTA; **D9** acronym chain + unlabeled lifecycle dots (legend/hover); **D17** GR/VI buttons → quiet links; **D11/D12** company/document detail drawer + inline status control; **D13** dead "Attach file (coming soon)" peers; **D4** Engineer dashboard "what do I do today" CTA; **D16** exec-dashboard clickable KPIs.

**▶ Wave 6 — Polish sweep (fold cheap items into whichever wave touches the same file).** A11y: **G3** rail-drawer `role=dialog`/focus-trap, **G4** tab `aria-controls`, **G5** focusable coming-soon explanations. Design-system: **H1** off-palette cyan KPITone, **H2** arbitrary px spacing, **H3** inline `hsl()` → tokens, **H4** off-scale fonts, **H5** mask `#000`, **H6** em-dash placeholders. IxD minors (Theme I): **I1** timesheet note-field demote, **I2/I3** incident date/type defaults, **I4** sales value-rule wording, **I5** dual search boxes, **I6** task-row open, **I7** doc ⋯ clip, **I8** SoD warning persists during edit, **I9** seed a Vendor-Invoiced PR, **I10** task dependencies decision (owner). Visual/taste (Theme J): **J1** budget-tab double hierarchy, **J2** sparse-page dead zones, **J3** incident browse affordance, **J4** finance/exec dashboards read generic (not a finance console), **J5/J6** empty-state space, mobile small text.
...
**⚑ Owner decision — RESOLVED (2026-06-09): Wave 3 = CORRECTNESS-FIRST** (silent-data/integrity bugs + lifecycle rework dead-ends + procurement authz gaps), **mobile = Wave 4** (near-term users are desktop). Build order within Wave 3: ✅ Part 1 = wrong-data bugs (F3/F4/F1/F2, PR #39 merged) → **Part 2** = resilience + RBAC-leak completion (F5/F6/F7/F8 + G2 + D10 + **N2/N3/N4** ⌘K/toolbar leaks + **O1** timesheet Submit-auto-saves) → **Part 3** = lifecycle dead-ends (B1/B2/B3+A8) → **Part 4** = procurement authz/correctness (A9 RPC over-grant + A10 + AC-W2-RBAC-011-RLS + **N1** two-payment-controls Blocker + **O3** co-locate VI capture).
...
- **NEW → Wave 3 (correctness/RBAC; the unfinished halves of Wave-2 Theme A + a money-control bug):**
  - **N1 (Blocker) Two competing payment controls** — the Vendor-Invoice form's status `<select>` still offers `Paid` (`ProcurementDetails.tsx:683-692`) alongside the authoritative SoD-gated **Mark as Paid** (`:163-164`); two ways to mark a PR paid, ambiguous authority on an audit-sensitive money step → drop `Paid` from the VI select (Mark-as-Paid sole authority).
  - **N2 RBAC leak** — Engineer sees the **"Approvals queue" toggle in the timesheet toolbar** (`Timesheets.tsx:420-429`, gated only on `may('create','timesheet')`); rail hides Approvals from Engineer but the toolbar doesn't → gate the toggle on approver role.
  - **N3 RBAC leak** — **⌘K Navigate group leaks** Sales/Procurement/Companies to Engineer (`App.tsx:189` `MODULES.map` unfiltered; the ⌘K *Records* cache was gated A-8, Navigate wasn't) → filter `MODULES` by real role.
  - **N4 ⌘K nav gaps** — **My Tasks unreachable via ⌘K** (`/my-tasks` not in `routeMatch.MODULES`) and ⌘K can't search by task name (`useRecordSearch` indexes projects/pipeline/procurements only) → add my-tasks to MODULES + (optionally) index tasks. Pairs with N3.
...
- **OWNER-DECISION items — RESOLVED 2026-06-09 (OD-W3-1..4 in decisions.md):**
  - **O1 → BUILD (Wave 3):** timesheet **Submit auto-saves valid dirty rows first** — keep both Save + Submit buttons, but Submit no longer requires a prior Save click (saves → submits → final confirm). The owner's originally-flagged flow; pairs with F5 (Timesheets.tsx). Submit gating must read the edit buffer, not only persisted entries (`Timesheets.tsx:360`).
  - **O2 → KEEP (no change):** procurement create stays the two-step (modal creates a Draft → detail page adds line items). A Draft PR is a legitimate editable/cancellable state; not worth the composer rework.
  - **O3 → BUILD (Wave 3, procurement):** **co-locate invoice capture with the Mark-Vendor-Invoiced action** (one step, mirroring the Mark-won inline contract-ref/date capture) — evidence-with-state, no "invoiced before the invoice". Pairs with N1 on `ProcurementDetails.tsx`.
  - **O4 → KEEP / DEFER (no change):** no inline pipeline stage-change; the detail page stays the single place to advance a deal. Drag-and-drop kanban is a sizeable build, revisit only if pipeline grooming becomes a pain.
