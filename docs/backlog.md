...
- **Silent-failure / data-integrity bugs (Theme F):** **F3/F4** numeric text silently coerced to `0` (project value, line-items, quotes → WRONG values persisted); **F1/F2** project/company mutations don't invalidate `['fk-options']` → 5-min stale/archived/missing FK pickers; **F8** create modals submit with blank required fields (no disable/first-error focus) — shared `EntityFormModal` readiness; **F5** timesheet delete optimistic, no rollback; **F6** budget mutations drop DB error codes; **F7** unknown URL → silent dashboard (add 404).
