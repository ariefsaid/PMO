...
>
> **⭐ IMMEDIATE NEXT ACTION — none queued; awaiting owner.** Prod current, branches in sync (`dev`==`main`==`production`==`fc312eb`). One non-blocking follow-up (chip `task_a5581daf`): dedup the GR/VI capture forms onto `RecordCaptureForm`, extract `ProcurementDecisionZone` from the large `ProcurementDetails.tsx`, fix `Tabs.tsx` roving focus — code-quality only, no behavior change. Pick from "Open feature tracks" or await a directive.
>
