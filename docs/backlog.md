...
>
> **⭐ IMMEDIATE NEXT ACTION — none blocking; `dev` is 2 commits ahead of `main` (the 2 follow-ups), optional promote.** **`main`=`7a65ac7` (PROMOTED 2026-06-21, owner "ship to main", PR #169, gated green)** now carries the full procurement IxD + Reserved-budget program (#162–168). **`dev`=`d317260` is 2 ahead** — the two now-DONE follow-ups: **#170** `0001_rls_enabled` made catalog-driven (no allowlist; all 35 public tables verified RLS+FORCE), **#171** the `vi-*` field testids single-sourced across both VI capture paths (`vendorInvoiceTestIds.ts`; shared-component extraction assessed + rejected as over-engineering — the two VI renderings genuinely differ in DOM/order/widths/a11y). Both `verify`-green on `dev`; promote them to `main` whenever (gated `verify`+`integration`). **`production` UNTOUCHED — `fc312eb`/mig 0041, now ~13 behind `main`; needs a direct per-instance owner go (would push mig 0042 to the cloud DB + FE to `production`).** **The 2026-06-21 program promoted to main (#162–168):**
...
>
> **The two follow-ups are now DONE (on `dev`):** (1) ✅ **#170** `0001_rls_enabled` catalog-driven (security-audit Low closed). (2) ✅ **#171** `vi-*` testids single-sourced (code-quality Minor closed). No open procurement debt.
>
