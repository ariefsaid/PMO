...
### ⚑ THE TRACKER IS THE BACKLOG — this file explains, it does not hold work

**Every actionable item here carries its issue number in its heading.** If you add work to this file
without an issue, it becomes invisible: a frontier query cannot see prose, and nobody greps a
1,700-line document for what to do next.

⛔ **This is not a style preference — it hid two OWNER questions for a month.** "Was dropping
bulk-approve deliberate?" (2026-07-27) and "the approvals queue and the authority disagree"
(2026-07-29) sat in this file as `### OPEN QUESTION (owner)` headings. The wayfinder frontier keys on
the **resolver label**, so neither existed as far as any session could tell. They are now
[#557](https://github.com/ariefsaid/PMO/issues/557) and
[#558](https://github.com/ariefsaid/PMO/issues/558), parked on map #450 with the three fields a park
requires (resolver label · `Map: #<n>` first line · native sub-issue link).

**So:** decision or question ⇒ a wayfinder ticket on the right map. Buildable work ⇒ an ordinary
issue. This file keeps the *reasoning* — why a thing is shaped the way it is, and what was already
tried — which is the part an issue title cannot carry.
...
### ⚑⚑ BACKLOG STALENESS — the 2026-07-23 audit, and the 2026-08-24 sweep that acted on it

> **2026-08-24: the sweep this section asked for was finally DONE.** Nine superseded status blocks —
> including two titled *"CURRENT STATUS — read first"* and *"RESUME HERE"* that asserted a
> `dev == main` six weeks out of date — were moved to `docs/history.md` § Archived status blocks. An
> agent reading this file top-down used to meet the stalest claim first.
>
> ⚑ **A staleness WARNING is not a fix.** This marker sat here for a month while the entries it
> warned about stayed in place and kept being read. If you find yourself adding a note that says
> "check before trusting", move or correct the thing instead.
...
### ⚑ ERPNEXT FOLLOW-UPS — ⛔ NOT BUILT, reserved migration numbers GONE — **tracked as [#565](https://github.com/ariefsaid/PMO/issues/565)**

> **Corrected 2026-08-24.** The heading said "in build". No branch for any of these exists on the
> remote (`feat/timesheet-reopen` is absent), so nothing was built.
>
> ⛔ **DO NOT USE THE MIGRATION NUMBERS BELOW.** This section reserves `0151`/`0152`/`0153`/`0154`
> and **all four were taken by unrelated shipped work** — `0151_m365_ciphertext_envelope_check`,
> `0152_platform_operators_revoke_client_writes`, `0153_budget_line_item_fiscal_year`,
> `0154_budget_identity_year_qualified`. Take the next free number when this is picked up;
> `scripts/renumber-migration.sh` exists for the collision this would otherwise cause.
>
> ⚑ **Reserving a migration number in prose is how that collision happens.** A number is only yours
> once the file is on a branch.
...
### ✅ LANDED — the authorization-hierarchy slice (was "IN FLIGHT"; corrected 2026-08-24)

> **This shipped as PR #418 (`2ff49731`) and is on `main`.** It sat under an "IN FLIGHT" heading for
> four weeks. The branch `fix/authz-hierarchy` still exists on the remote and is stale — see the
> stale-branch note in § Standing debt. Detail below kept for provenance.
...
### ⚑ DEBT — AC-ERR-001 wired on 2 of 18 forms — **tracked as [#559](https://github.com/ariefsaid/PMO/issues/559)** (2026-07-28)
...
### ⚑ DEBT — AC-IXD-PROC-W5-3 stale locator, wrong skip reason — **tracked as [#560](https://github.com/ariefsaid/PMO/issues/560)**
...
### ⚑ OWNER QUESTION — bulk-approve dropped from the main approvals view — **PARKED as [#557](https://github.com/ariefsaid/PMO/issues/557)** on map #450
...
### ⚑ OWNER QUESTION — the approvals queue and the approval AUTHORITY disagree — **PARKED as [#558](https://github.com/ariefsaid/PMO/issues/558)** on map #450
...
### ⚑ PARKED — mutation testing (StrykerJS) — **tracked as [#561](https://github.com/ariefsaid/PMO/issues/561)** (2026-07-25)
...
### ⚑ ERPNext operational-completeness slate — **tracked as [#562](https://github.com/ariefsaid/PMO/issues/562)** (NOT scheduled)
...
### ⚑ OPEN feature tracks (owner-scope-gated — NOT started) — entitlements **tracked as [#563](https://github.com/ariefsaid/PMO/issues/563)**
...
### ⚑ PER-USER CREDIT ALLOCATION — **tracked as [#564](https://github.com/ariefsaid/PMO/issues/564)**, NOT BUILT
