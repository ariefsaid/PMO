...
- **`main`→`production`** stays a SEPARATE, owner-instructed-per-instance promote (never autonomous). Migrations to push to prod Cloud then: `0076–0080` (prod was ~`0060`).

### In progress — the audit HIGHS (owner-directed, 2026-07-07, glm-5.2/4.7)
1. **feature-flag server-enforcement** — flags are FE-only; conjoin `org_has_feature`/`org_features` into the `*_write` RLS policies so a disabled feature can't be written server-side. (glm-5.2, RLS.)
2. **orphaned-Auth-user compensation** — `admin-invite-user` creates the Auth user then inserts the profile; roll back the Auth user if the profile insert fails (no orphaned auth.users). (glm-5.2, edge-fn.)
3. **e2e blindspots** — fail LOUD when `SERVICE_ROLE_KEY` absent (2 specs silent-skip → report green) + a guard that flags the 4 hard-skipped Incidents-quarantine e2e on flag-flip. (glm-4.7, test infra.)
...
- **Auditor gaps still open:** e2e blindspots (4 hard-skipped Incidents-quarantine + 2 `SERVICE_ROLE_KEY`-absent silent-skips report green) · telegram-notify send-ok+stamp-fail dup alerts (`index.ts:86`) · `notifyOwner` swallows errors untraced · health endpoint checks zero deps · `enforce_automation_owner_cap` racy count-then-insert (SHARE ROW EXCLUSIVE pattern at `0065:69`) · `set_project_contract_value` accepts negative (overlaps money `CHECK(>=0)`; #17 logs but doesn't reject) · `spike-rls.yml` `npm install`+service-role-key (pin+ci or delete) · 3 missing runbooks (prod-deploy/secret-rotation/agent-LLM-outage — doc conversions from `environments.md`).
- **Earlier-audit Highs/Meds (not started):** feature-flag server-enforcement (conjoin `org_has_feature` into `*_write` RLS) · orphaned-Auth-user compensation (`admin-invite-user`) · agent-persistence stuck-`running` · interactive-create idempotency · `error_events` completeness + retention · S-curve today-position test · PostHog consent-gate · agent-chat rate-limit.
