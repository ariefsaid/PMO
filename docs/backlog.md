...
**▶ Wave 3 — ✅ CORE COMPLETE (correctness, integrity & lifecycle). Only PR-F (F8/A8/B3) remains.** PRs #39–#44 all merged to `main` (`main`@5ff5ab9). Build order Part 1→4 shipped as 6 PRs:
- **Silent-failure / data-integrity bugs (Theme F):** ✅ **F3/F4** numeric→0 silent coercion (shared `parseMoneyInput`, validate==persist) + **F1/F2** stale fk-options pickers (`['fk-options']` invalidation) — **DONE, PR #39** (`main`@21c58b2). ✅ **F5** timesheet delete rollback on failure + **F6** budget DAL preserves Postgres error codes (`toAppError`, incl supabase plain-object errors) + **F7** unknown URL → `NotFound` page — **DONE, PR #40** (`main`@b38c2cd). Remaining: **F8** create modals submit with blank required fields (no disable/first-error focus) — shared `EntityFormModal` readiness → **PR-F**.
- **Lifecycle rework dead-ends (Theme B):** ✅ **B1** timesheet Rejected→Draft "Revise" control + **B2** document Rejected→Draft/Closed actions — **DONE, PR #42** (`main`@3c6dcd4). Remaining: **B3** Admin break-glass procurement moves in the FE (now safe — migration `0018` enforces SoD on Admin server-side) → **PR-F**, with **A8** Admin edit Draft/Rejected PR header.
