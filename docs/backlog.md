...
1. **▶ Timesheet entry + edit — IN PROGRESS** (SDD→TDD→BDD; spec `docs/specs/timesheet-entry.spec.md`). Engineers create/edit/delete/submit their own weekly time entries (grid was read + submit-only). **Owner scope decisions (locked):** inline editable weekly grid (per-project rows × per-day cells); project picker = **any active org project now** (assigned-only = post-MVP follow-up); **Draft-only editing** (manager rejects/returns a Submitted sheet to re-edit). Foundation already exists: `timesheet_entries` table + a `timesheet_entries_write` RLS policy (own+Draft via USING) — **but its INSERT/UPDATE `WITH CHECK` only verifies `org_id` (a hole: could write an entry onto another user's same-org sheet); the build MUST tighten it to own+Draft.** Delete = confirm dialog; Save = explicit (form-style, no per-keystroke write). First write-feature of the wave.
