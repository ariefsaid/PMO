...
**▶ Wave 6 — Polish sweep (fold cheap items into whichever wave touches the same file).** A11y: **G3** rail-drawer `role=dialog`/focus-trap, **G4** tab `aria-controls`, **G5** focusable coming-soon explanations, **G6** disabled-submit affordance hint (when the F8 `isComplete` gate disables `EntityFormModal` submit on blank required fields, give a programmatic reason — required `*` markers + `aria-describedby` on submit — since the error-summary path only fires on a submit attempt, which is blocked while blank; deferred from PR-F, code-quality Minor #1). Design-system: **H1** off-palette cyan KPITone, **H2** arbitrary px spacing, **H3** inline `hsl()` → tokens, **H4** off-scale fonts, **H5** mask `#000`, **H6** em-dash placeholders. IxD minors (Theme I): **I1** timesheet note-field demote, **I2/I3** incident date/type defaults, **I4** sales value-rule wording, **I5** dual search boxes, **I6** task-row open, **I7** doc ⋯ clip, **I8** SoD warning persists during edit, **I9** seed a Vendor-Invoiced PR, **I10** task dependencies decision (owner). Visual/taste (Theme J): **J1** budget-tab double hierarchy, **J2** sparse-page dead zones, **J3** incident browse affordance, **J4** finance/exec dashboards read generic (not a finance console), **J5/J6** empty-state space, mobile small text.
