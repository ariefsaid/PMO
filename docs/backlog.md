...
- **DONE on `dev` (flag-gated, NOT promoted):** I1 safe markdown (`f970a14`), I2 layered prompt/skills
  ...
  Track E surfacing specs (`AC-AXP-011/012/013/014/016` Playwright specs added and `--list` verified), and
  **I5 Cmd+K + conditional approvals** (`AC-AT2-006..013` unit proofs + `AC-AT2-007` Playwright spec listed).
  Latest continuation commits update this handoff + the relevant plan progress sections. Wave-1 review battery
  green (security: no C/H/M; one code-quality Important fixed).
- **NEXT (for the resuming agent), in order:** Tier-2 per the tier-2 spec — **I6** eval harness (also the
  model-quality gate) → **I4** attachments (biggest new-build) → **I7** obs-memory (DEFERRED behind a
  token-cost trigger). Track E/I5 browser
