...
- **▶ fix-wave-3 IN PROGRESS (plan-of-record in the r3 doc):**
  ...
  - **▶ G3d NEXT (last build group):** mobile overflow (PageHeader/BvACard @390 + ProgressBar cn) + ROLE_PILL/VERSION_PILL→registry + noun fix + dead-code delete + guard tests + xlsx-export catch.
