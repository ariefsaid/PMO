...
- **Transition bug** (owner-reported "clicked but status didn't change") root-caused: UI `allowedActions` is computed from the impersonated `effectiveRole`, but the RPC authorizes against the real-JWT `auth_role()`; plus the SoD-b "cosmetically allow Mark-Paid to the approver" (ProcurementDetails ~108-111) → RPC throws `42501`/`P0001` but the error renders only as a buried inline alert + the pill never moves. **Fix (folded into stage 5):** surface every transition error as a toast (classify `P0001`=illegal-stage / `42501`=not-permitted/SoD), preserve `error.code` through `procurementLifecycle.ts`+`useProcurementDetail.ts`, tighten `allowedActions` to match the RPC. RPCs (migration `0006`) UNCHANGED.
