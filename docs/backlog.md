...
### Next
- **#14 supply-chain/CI — the LAST outstanding audit item.** DONE on branch `harden/supply-chain-ci` (6 `deno.lock` + version pins + `--frozen` CI gate; 12 Actions SHA-pinned; new pgTAP-on-PR→dev job). NOT merged — **rebase onto current `dev`** (which moved a lot), resolve `ci.yml`, merge. (While rebasing: confirm the lockfile regen records the uuid v11 override — auditor SC-Med #5.)
- **Promote `dev`→`main`** (Director-gated) once #14 lands — runs the `integration` lane (all pgTAP + e2e + visual) that PR→dev skips. Then prod is owner-gated per below.
