...
>
> **⭐ IMMEDIATE NEXT ACTION — `dev → main` promote PENDING owner go.** Prod/main current at `fc312eb`/mig 0041; **`dev`=`00418af` is 9 commits AHEAD** — a fully-reviewed **procurement IxD + Reserved-budget program** (2026-06-21 session). Promote = open a PR `dev → main` (gated CI: `verify` + `integration` = pgTAP + e2e + visual). **No `production` promote** unless owner says so per-instance. **The 9 ahead (all merged to `dev` + reviewed):**
...
>
> **Two low-priority follow-ups (chips dismissed by owner — tracked here so they're not lost):** (1) **make `supabase/tests/0001_rls_enabled.test.sql` catalog-driven** like `0005` — a future table shipped with RLS *entirely off* currently escapes both tests (real, dormant gap; security-audit Low). (2) **dedup `VIInlineCapture` vs `RecordCaptureForm kind="vendor_invoice"`** — they share `data-testid`s (mutually-exclusive, not a bug, but a drift trap; code-quality Minor). Neither blocks the promote.
>
