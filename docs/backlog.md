...
## ▶ Current state (2026-06-17) — PROD CURRENT: mobile + PostHog + S-curve + row-clickable + Vite 8 + timesheet-icon-fix + Gantt-single-scroll-fix LIVE; tiered CI + visual gate

> **RESUME ENTRY POINT (model-agnostic).** **Prod is CURRENT** — `production`=`main`=`dev`=`5c6f5d7` / Cloud DB **migration 0034**. FE promotes this session: `5ce5a39` (mig 0034 + mobile/PostHog/S-curve/row-click/Vite8) → `d3d50b0` (timesheet icon fix #144) → `5c6f5d7` (Gantt single-scroll fix #149/#150). Migration 0034 self-backfills existing Done rows; no reseed needed.
> **⚑ BINDING (owner 2026-06-17): work→`dev`→`main`; `main` is the autonomous ceiling. NEVER promote to `production` (FE push or `db-push-prod.sh`) without a DIRECT per-instance owner instruction.** (The 5c6f5d7 prod promote was such a direct instruction.)
>
> **⭐ IMMEDIATE NEXT ACTION — none queued; awaiting owner.** All in-flight PRs merged, prod current, branches in sync (`dev`==`main`==`production`==`5c6f5d7`). Optional follow-ups in OUTSTANDING. Pick from "Open feature tracks" or await a directive.
>
