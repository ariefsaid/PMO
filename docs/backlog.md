...
> **RESUME ENTRY POINT (model-agnostic).** `main` @ `a1e5115`. **Prod PROMOTED 2026-06-16** — `production` = `a1e5115`, Cloud DB at **migration 0033** (0028–0033 pushed), FE building on Cloudflare (https://pmo-bfb.pages.dev), and prod **re-seeded clean** with the latest demo dataset (truncate + fresh `seed.sql` — prod had stale `d0…`-id data; see `docs/environments.md` § Seeding prod). main == production now. Authoritative self-contained handoff: **this block + `docs/qa-portfolio.md` (the QA model) + `docs/reviews/2026-06-16-qa-orchestration-trial-gantt.md` (the pi/GLM trial)**. Everything below the "⟨SHIPPED & SUPERSEDED⟩" header is HISTORY.
...
0. **✅ DONE (2026-06-16): GANTT v2 RENDER-VERIFIED — PASS.** Director-run live render check (`npm run dev` + local Supabase, Meridian Steelworks seed: 4 milestones / ~16 dated tasks) confirmed on desktop @1440 + mobile @390: on-axis milestone diamonds + dotted guides, dependency connectors/arrowheads (elbow), MS-Project split, Day/Week/Month/Quarter zoom, Today line, narrow-bar label suppression, **and the 390px mobile fallback notice + working List/Board switch** (the owed residual). 52/52 Gantt unit/layout tests green. Report: `docs/reviews/2026-06-16-gantt-v2-render-verification.md`. **One incidental finding (NOT Gantt):** the **S-Curve** recharts time-axis emitted an intermittent React duplicate-key warning (`tick-<ts>-44-44`) — dev-only, prod-stripped, no visual break; flagged for a separate fix (explicit de-duplicated `ticks` + regression test, graduate per ADR-0030). **Local env gotcha:** `node_modules` was stale (`date-fns@4.4.0` from #130 uninstalled) → `npm install` + Vite restart required before the app runs.
1. **Vendoring:** **date-fns ✅ MERGED #130** (OD-DATE-1, TZ bug class killed). **TanStack Table ✅ ASSESSED → DEFER #131** (`DataTable` is controlled/presentational — no engine to replace; RESCOPE only on a real pagination/multi-select/pinning driver). Vendoring backlog effectively closed for now.
2. **✅ PROD PROMOTE DONE (2026-06-16).** Migs 0028–0033 pushed to Cloud (`db-push-prod.sh`); prod re-seeded clean with latest demo data (new `scripts/db-seed-prod.sh` + a one-off truncate — prod held stale older-seed data the idempotent seed couldn't overlay); FE `main`@`a1e5115` → `production` (Cloudflare building). Prod now: 17 projects / 51 tasks / 15 contacts / 24 CRM activities / 20 procurements, id scheme matches current seed. **Residual debt (non-blocking, fold later):** the 2 LOW pgTAP regression assertions on migration 0028 (procurement_*_files org-override + anon-read), and the 1 dependabot-high (esbuild, dismissed not-affected). The demo-seed-to-prod posture is documented in `docs/environments.md` (delete `db-seed-prod.sh` if prod ever holds real tenant data).
3. **Minor doc residuals:** breakpoint-doc 768-vs-640; `qa-portfolio.md` L3 no-server-sandbox recipe + L0 exact date-fns target files. Low value.
