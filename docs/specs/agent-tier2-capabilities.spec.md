...
| 7 | Chat attachments | **SDD AUTHORED — build pending (2026-07-05)** | ADR-0053 + plan `docs/plans/2026-07-05-agent-chat-attachments.md` authored (the `agent_attachments` table + bucket + provider seam + transcode + untrusted-input boundary + two model paths). Build is the next issue-loop; load-bearing owner-confirmable is the PDF-extraction stack (§OQ-3 / DEC-8). Existing Storage pattern to mirror: `useFileUpload.ts` + `repositories.document.prepareUpload/confirmUpload/cleanupObject`. |
| 8 | Cmd+K "Ask AI" fallback | **PARTIAL — wiring only** | Palette exists: `CommandPalette.tsx` renders a `No results for "{query}"` empty state (L244–247) but **no "Ask AI" affordance**. Items are built in `App.tsx` `paletteItems` (L307–323); open state is `App.tsx` `paletteOpen`. Panel-open API exists: `AgentRuntimeProvider.tsx` `openPanel()` (L46) — **but takes no prefill arg**. Composer draft state lives in `AssistantPanel.tsx` `composerValue`/`setComposerValue` (L131) — **not exposed for external prefill**. So: the seams exist, the wiring (query→openPanel(prefill)→composer) and the prefill parameter do not. |
| 8 | Contextual suggestion chips | **MISSING** | No suggestion/prompt-chip component in `pmo-portal/src/components/panel` (`grep suggestion\|promptChip` → none). `EmptyState.tsx` exists but renders no route-aware prompts. Live route/entity context already flows to the handler (ADR-0045 §3, `RunContext.entity`) — the chip's *content* seam exists; the chip UI does not. |
| 11 | Conditional approvals | **PARTIAL — static boolean today** | `AgentAction.confirm?: boolean` is a **fixed field per action** (`port.ts` L102; every action in `actions.ts` sets `confirm:` a literal — L175/222/282/319/444/542/561). The handler routes on it (`handler.ts` A3 "Propose branch" L737, `dispatchAction` guard L412–422, `isConfirmAction` L1509). **No predicate, no materiality threshold, no args/context input.** The chip UX to *display* an approval is fully built (`ApprovalChip.tsx`, `NeedsApprovalPayload` in `port.ts`). |
| 10 | Agent eval harness in CI | **MISSING (new-build)** | No `*.eval.ts` files (`find` → none); no `usesTool`/`llmJudge`/scorer module (`grep` → none). CI has `verify` + `integration` jobs (`.github/workflows/ci.yml`); **edge functions do not run in CI** — `integration` disables `edge_runtime` in `config.toml` and only `deno check` + a boot-smoke run over `supabase/functions/**` (`ci.yml` L108–134, L166). This is the load-bearing constraint for where evals can run (see FR-AT2-EV + Open Questions). |
...
auto-send. With no entity context, no chips render.
