...
| **`adr/`** | Architecture Decision Records `0001–0042` (no 0013; no 0026 — the 0026 migration was a bug-fix RPC, below the ADR threshold). Latest: `0039` PMO-native agent architecture + untrusted-output validation boundary, `0040` the in-app agent panel (PMO-native conversational surface), `0041` model-calling agent actions (capability seam beyond `DeputyContext`) — all **Accepted & shipped** with the agent-native epic A1–A4 (PR #200, on `main`); `0042` **versioning & release convention** (SemVer pre-1.0; the bump rule + release-manifest for future agents) — **Accepted**. Earlier: `0036` agent-native user-composed UI, `0037` query-spec DSL + RLS-scoped compiler, `0038` view-renderer executor — **Accepted**. The durable "why" of every architectural choice. `0006` (hosting) is **ACCEPTED** (deployed). |
