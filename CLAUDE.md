...
- **Branch flow (binding, owner directive 2026-06-17):** **work lands on `dev` → promoted to `main` (gated). `main` is the ceiling for autonomous work.** **NEVER promote to `production` — no `git push origin main:production`, no prod DB push (`db-push-prod.sh`), no prod reseed — unless the owner instructs it DIRECTLY, that time.** A prior "ship to prod" is per-instance, never standing. CI is tiered to match (lean — no duplicate runs): PR→`dev` = `verify` only (fast lane); PR→`main` = `verify` + `integration` (pgTAP + e2e + visual gates) so `main` is always clean; push to `main` = `verify` smoke only. Push CI runs on `main` ONLY (not `dev`/feature branches — they're PR-gated, so a dev push-trigger would just re-`verify` already-verified content); `integration` fires exactly once per change (the PR→`main`). `main`→`production` is a manual, owner-instructed promote only.
