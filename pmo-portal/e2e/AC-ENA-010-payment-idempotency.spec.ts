...
test.describe('AC-ENA-010: after-commit-before-mirror fault-seam interruption — a retry never duplicates the ERP money doc', () => {
  test('the first (faulted) attempt commits the real PE and crashes before the mirror; the retry reconciles (finalize-only, no second POST); exactly one ERP doc + one payments row', async () => {
    ...
    try {
      ...
      // exists with the returned name (a live-bench GET, never faked). NOTE (live-bench finding,
      // 2026-07-12): Payment Entry's `remarks` field is SERVER-SIDE OVERWRITTEN by ERPNext's own
      // `validate` hook (an auto-generated "Amount X to Y..." description) on every save — the
      // idempotency-key stamp (`stampRemarks`) is silently clobbered, so a `remarks`-filter probe can
      // never find it for THIS doctype specifically (the field IS queryable, `remarksQueryable:true`
      // in doctypeRegistry.ts remains accurate for the REST-permission sense, but the STAMPED VALUE
      // never survives). This does NOT weaken R1 (the DB-enforced atomic claim, proven above by the
      // single-mirror-row + matching-externalRecordId assertions, is the actual concurrent-duplicate
      // guarantee) nor the exact scenario this test proves (a `committed`-state crash reconciles via
      // finalize-only, which never calls the probe at all) — it DOES mean R3's probe-based orphan
      // recovery for a `pending`/`failed`-state Payment Entry crash (a deeper interruption than this
      // test models) cannot self-heal via remarks for this doctype; flagged for the Director as a
      // follow-up (a different stock anchor, or accepting the narrower gap, needs a decision — out of
      // this task's fix authority). ──
