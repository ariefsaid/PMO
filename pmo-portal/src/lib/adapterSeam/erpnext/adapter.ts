...
async function commitErpCommand(command: AdapterCommand, deps: ErpAdapterDeps): Promise<CommandResult> {
  ...
  if (command.operation === 'update') {
    ...
    // wired into commit() in slices 4/6 — a loud throw here, never a silent no-op.
    throw new AdapterError('commit-rejected', `erpnext adapter update for submittable kind '${kind}' is wired in slices 4/6`);
  }
  // 'transition': submit/cancel/amend for submittable kinds lands in slices 4/6. A loud throw here.
