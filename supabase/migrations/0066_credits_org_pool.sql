-- 0066_credits_org_pool.sql — credits org-pool refactor (FR-CRE-001/003, AC-CRE-002). NON-DESTRUCTIVE.
...
--     (0067) / operator_usage_summary (FR-OPR-004).
...
-- retained as defense-in-depth (0063's conjunction is superseded here by an explicit restatement).
...
create policy credits_insert on public.credits for insert
  with check (
    ...
    and owner_id is null                    -- security review L3: a direct INSERT can't forge attribution; matches operator_grant_credits (0067 writes owner_id NULL)
  );
