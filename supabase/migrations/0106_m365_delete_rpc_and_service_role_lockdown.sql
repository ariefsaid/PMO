-- 0106_m365_delete_rpc_and_service_role_lockdown.sql — Luna round-4 fixes (MED-2 + the delete-side
...
--     (auto-granted by 0080's ALTER DEFAULT PRIVILEGES when 0096 created the table). Every edge-fn
--     connection mutation already routes through the 0105 lock-order RPCs EXCEPT revoke.ts, which
...
--           0105/0106 RPCs are the ONLY mutation path a service-role caller can take. Reads (SELECT)
--           are untouched — the edge fn selects connection rows (proxy.ts / revoke.ts load).
--     SECURITY DEFINER functions (the 0105/0106 RPCs, _m365_disconnect_cascade_core, the write-guard
...
--
-- GLOBAL LOCK ORDER (binding — unchanged from 0105):
...
--    revoke.ts treats null|error as failure (NOT_CONNECTED / INTERNAL_ERROR) — mirrors H6 + the 0105
--    null|error = failure contract. The caller audits; this RPC does not (same as the 0105 RPCs).
...
declare
  v_id uuid;
begin
  -- GLOBAL LOCK ORDER: PROFILES → ORG_FEATURES → ms_graph_connections (see 0105 file header).
...
--    ms_graph_connections. The SECURITY-DEFINER RPCs (0105 §1/§2/§3 + 0106 §1) are the only mutation
...
--    This revokes whatever 0080's ALTER DEFAULT PRIVILEGES auto-granted when 0096 created the table;
