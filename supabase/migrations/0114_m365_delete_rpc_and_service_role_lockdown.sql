...
--     (auto-granted by 0080's ALTER DEFAULT PRIVILEGES when 0104 created the table). Every edge-fn
--     connection mutation already routes through the 0113 lock-order RPCs EXCEPT revoke.ts, which
...
--           0113/0114 RPCs are the ONLY mutation path a service-role caller can take. Reads (SELECT)
--           are untouched — the edge fn selects connection rows (proxy.ts / revoke.ts load).
--     SECURITY DEFINER functions (the 0113/0114 RPCs, _m365_disconnect_cascade_core, the write-guard
...
--
-- GLOBAL LOCK ORDER (binding — unchanged from 0113):
...
--    revoke.ts treats null|error as failure (NOT_CONNECTED / INTERNAL_ERROR) — mirrors H6 + the 0113
--    null|error = failure contract. The caller audits; this RPC does not (same as the 0113 RPCs).
...
declare
  v_id uuid;
begin
  -- GLOBAL LOCK ORDER: PROFILES → ORG_FEATURES → ms_graph_connections (see 0113 file header).
...
--    ms_graph_connections. The SECURITY-DEFINER RPCs (0113 §1/§2/§3 + 0114 §1) are the only mutation
...
--    This revokes whatever 0080's ALTER DEFAULT PRIVILEGES auto-granted when 0104 created the table;
