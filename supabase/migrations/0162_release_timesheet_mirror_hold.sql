...
-- ============================================================================
-- ⚑ MERGE-TIME UNION (integration/erpnext-followups, 2026-07-24): this migration is renumbered to run
-- AFTER FU-2's 0156, so it is the FINAL `release_outbox_hold` definition and MUST carry both lanes'
-- changes — FU-1a's timesheet-mirror CAS (below) AND FU-2's optional p_expected_domain guard (folded in
-- inline). The 2-arg form is dropped so every 2-arg call (`release_outbox_hold(id, reason)`) binds to the
-- 3-arg default and no `42725` ambiguity arises. Record: docs/handoffs/2026-07-24-release-outbox-hold-reconciliation.md.
...
begin
  ...
  -- FU-2 (0156) domain guard, folded into the union: an optional caller-declared expected domain. NULL
  -- (the timesheet lane's 2-arg-shaped call) asserts nothing — byte-for-byte the shipped behaviour. A
  -- non-null mismatch means the caller is releasing a command in a domain it did not expect ⇒ refuse.
