...
-- Σprompt_tokens, Σcompletion_tokens, Σprovider_cost_usd, Σcost per (owner_id, action, month)
-- exactly, plus margin_usd (null when CREDITS_PER_USD unset).
begin;
select plan(6);
...
-- 'chat' aggregate: run_count=2, prompt=30, completion=13, provider_cost_usd=0.05, cost=0.12.
select is(
...
select is(
  (select provider_cost_usd from org_usage_summary() where action = 'chat'),
  0.05::numeric, 'AC-USE-002 chat Σprovider_cost_usd = 0.05');
select is(
