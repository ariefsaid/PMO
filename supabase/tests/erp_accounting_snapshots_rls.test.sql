...
insert into organizations (id, name) values
  ('01000000-0000-0000-0000-000000000001','AC-ENA Snapshots A'),
  ('01000000-0000-0000-0000-000000000002','AC-ENA Snapshots B');
insert into auth.users (id, email) values
  ('01000000-0000-0000-0000-0000000000a1','snap-a@example.com'),
  ('01000000-0000-0000-0000-0000000000b1','snap-b@example.com');
insert into profiles (id, org_id, full_name, email, role, status) values
  ('01000000-0000-0000-0000-0000000000a1','01000000-0000-0000-0000-000000000001','A','snap-a@example.com','Admin','active'),
  ('01000000-0000-0000-0000-0000000000b1','01000000-0000-0000-0000-000000000002','B','snap-b@example.com','Admin','active');
...
values
  ('01000000-0000-0000-0000-000000000001','ACC-GLE-001','Creditors - PSC','Main - PSC','2026',null,'Supplier','Spike Supplier',
   'Purchase Invoice','ACC-PINV-2026-00018','2026-07-12',0,50000,false,1,'2026-07-12 12:00:00'),
  ('01000000-0000-0000-0000-000000000001','ACC-GLE-002','Stock Received But Not Billed - PSC','Main - PSC','2026',null,null,null,
...
select throws_ok(
  $$ insert into erp_gl_entry_mirror (org_id, erp_name, account, erp_modified)
       values ('01000000-0000-0000-0000-000000000001','ACC-GLE-001','Creditors - PSC','2026-07-12 12:00:00') $$,
  '23505', null,
  'AC-ENA-060 erp_gl_entry_mirror unique(org_id,erp_name) rejects a re-fed duplicate row');

set local role authenticated;
set local request.jwt.claims = '{"sub":"01000000-0000-0000-0000-0000000000a1","role":"authenticated"}';
select is((select count(*)::int from erp_gl_entry_mirror), 2,
  'AC-ENA-060 org-A member reads own-org GL-mirror rows');
set local request.jwt.claims = '{"sub":"01000000-0000-0000-0000-0000000000b1","role":"authenticated"}';
...
select throws_ok(
  $$ insert into erp_gl_entry_mirror (org_id, erp_name, account, erp_modified)
       values ('01000000-0000-0000-0000-000000000002','ACC-GLE-X','Acct','2026-07-12') $$,
  '42501', null,
...
values ('01000000-0000-0000-0000-000000000001','ACC-PLE-001','Creditors - PSC','Supplier','Spike Supplier',
...
select throws_ok(
  $$ insert into erp_payment_ledger_mirror (org_id, erp_name, account, erp_modified)
       values ('01000000-0000-0000-0000-000000000001','ACC-PLE-001','Creditors - PSC','2026-07-12') $$,
  '23505', null,
...
values
  ('01000000-0000-0000-0000-000000000001','Main - PSC','Creditors - PSC','2026',0,50000,-50000,'GL Entry','11111111-0000-0000-0000-000000000001'),
  ('01000000-0000-0000-0000-000000000001','Main - PSC','Stock Received But Not Billed - PSC','2026',50000,0,50000,'GL Entry','11111111-0000-0000-0000-000000000001');
set local role authenticated;
set local request.jwt.claims = '{"sub":"01000000-0000-0000-0000-0000000000a1","role":"authenticated"}';
...
reset role;
begin;
  delete from erp_actuals_snapshot where org_id = '01000000-0000-0000-0000-000000000001';
  insert into erp_actuals_snapshot (org_id, cost_center, account, fiscal_year, debit, credit, net, source_report, snapshot_id)
  values ('01000000-0000-0000-0000-000000000001','Main - PSC','Creditors - PSC','2026',0,75000,-75000,'GL Entry','22222222-0000-0000-0000-000000000002');
commit;
select is((select count(*)::int from erp_actuals_snapshot where org_id = '01000000-0000-0000-0000-000000000001'), 1,
  'AC-ENA-060 a refresh replaces the prior scope (single snapshot_id / single row after refresh, not append)');
select is((select count(distinct snapshot_id)::int from erp_actuals_snapshot where org_id = '01000000-0000-0000-0000-000000000001'), 1,
...
values ('01000000-0000-0000-0000-000000000001','Spike Supplier','Supplier','IDR',75000,0,75000,0,0,0,
...
select throws_ok(
  $$ insert into erp_ap_aging_snapshot (org_id, party, party_type, snapshot_id)
       values ('01000000-0000-0000-0000-000000000002','X','Supplier', gen_random_uuid()) $$,
  '42501', null,
  'AC-ENA-061 user-JWT INSERT into erp_ap_aging_snapshot denied (machine-written only)');
reset role;
begin;
  delete from erp_ap_aging_snapshot where org_id = '01000000-0000-0000-0000-000000000001';
  insert into erp_ap_aging_snapshot (org_id, party, party_type, currency, total_outstanding, current, b_0_30,
    b_31_60, b_61_90, b_90_plus, range_labels, report_date, ageing_based_on, source_report, report_version, snapshot_id)
  values ('01000000-0000-0000-0000-000000000001','Spike Supplier','Supplier','IDR',60000,0,60000,0,0,0,
    '{"range1":"0-30","range2":"31-60","range3":"61-90","range4":"91-120"}'::jsonb,'2026-07-12','Due Date',
    'Accounts Payable','erpnext-15.94.3/frappe-15.96.0','44444444-0000-0000-0000-000000000004');
commit;
select is((select count(distinct snapshot_id)::int from erp_ap_aging_snapshot where org_id = '01000000-0000-0000-0000-000000000001'), 1,
...
insert into erp_ar_aging_snapshot (org_id, party, party_type, currency, total_outstanding, current, b_0_30,
  b_31_60, b_61_90, b_90_plus, range_labels, report_date, ageing_based_on, source_report, report_version, snapshot_id)
values ('01000000-0000-0000-0000-000000000002','Cust A','Customer','IDR',0,0,0,0,0,0,
  '{"range1":"0-30","range2":"31-60","range3":"61-90","range4":"91-120"}'::jsonb,'2026-07-12','Due Date',
