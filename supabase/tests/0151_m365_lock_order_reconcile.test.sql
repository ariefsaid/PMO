...
-- would seed legacy rows against the PRE-hardening (0112-era) schema, then APPLY 0113/0115 and
...
-- seeded data within a single test transaction. The 0113/0115 migrations also contain irreversible
...
-- Reproduces the OLD (0112-era) looser tenant CHECK so a legacy 'foo..bar' row is seedable, then
-- re-runs the EXACT corrected 0113 §5a(ii) preflight + re-adds the §6 CHECK. Proves 0113 no longer
...
-- Revert the tenant CHECK to the looser 0112-era form so 'foo..bar' is seedable (legacy data).
...
-- the looser 0112-era CHECK restored above; the write-guard is disabled for the seed as for foo..bar.
alter table public.ms_graph_connections disable trigger m365_connection_write_guard;
insert into public.ms_graph_connections
  (org_id, user_id, entra_tenant_id, scopes, refresh_token_ciphertext, key_id, status)
...
-- the foo..bar row would have survived and this ALTER would have ABORTED (→ 0113 never installed).
...
$$, 'AC-M365-163: the tightened tenant CHECK adds cleanly after the scrub (0113 no longer aborts on a populated DB)');
select is(count(*)::int, 1, 'AC-M365-163 LOW-4: the .foo row survives AND passes the tightened CHECK (no data loss on upgrade)')
  from public.ms_graph_connections where entra_tenant_id = '.foo';

-- ============================================================================
-- AC-M365-164: the 0115 lock-order connection-mutation RPCs. Shape + grants + the deterministic
...
select is(count(*)::int, 3, 'AC-M365-164: the three 0115 connection-mutation RPCs exist in public')
  from pg_proc
...
-- ============================================================================
-- AC-M365-166: the one-time reconcile scrub (0115 §4). Seed a survivor (active+entitled) and two
