...
-- AC-1110: Tender Submitted stage — count=1, total_value=1200000, win_prob=0.500, weighted=600000
select ok(
  (select (elem->>'count')::int = 1
     and  abs((elem->>'total_value')::numeric - 1200000) < 1
     and  abs((elem->>'win_probability')::numeric - 0.5) < 1e-6
     and  abs((elem->>'weighted_value')::numeric - 600000) < 1
   from json_array_elements((get_sales_pipeline()->'stages')) elem
   where elem->>'status' = 'Tender Submitted'),
  'AC-1110: Tender Submitted stage count=1, total=1200000, win_prob=0.500, weighted=600000 (FR-SPD-010)'
);
