...
// ============================================================================
// #478 (0187/0188) — the SI mirror must carry the currency and the tax facts. `tax_treatment` and
// `tax_amount` are NOT NULL with no DB default, so a create that omits them fails outright; and a
// status-sync update must never write null over a recorded currency/tax figure.
// ============================================================================

Deno.test({
  name: '#478 — a created SI mirror carries currency, tax_amount, tax_template and tax_treatment=inclusive (amount IS the ERP grand_total)',
  fn: async () => {
    const { client, calls } = makeFakeClient({ companies: { org_id: 'org-1' }, projects: { org_id: 'org-1' } });
    const writer = getReadModelWriter('revenue');
    await writer.upsert(
      ...
      {
        id: 'pmo-si-478-1', si_number: 'ACC-SINV-2026-00478', amount: '111000.00',
        erp_outstanding_amount: '111000.00', erp_docstatus: 0, erp_modified: '2026-08-19 10:00:00.000000',
        currency: 'IDR', tax_amount: '11000.00', tax_template: 'Indonesia PPN 11% - RIS',
      },
      { domain: 'revenue', operation: 'create', record: { id: 'pmo-si-478-1', projectId: 'proj-1', customerId: 'cust-1', erp_doc_kind: 'sales-invoice' } },
    );
    ...
    const row = insertCall!.args[0] as Record<string, unknown>;
    assertEquals(row.currency, 'IDR', 'the mirror preserves the SOURCE doc currency, never a PMO constant');
    assertEquals(row.tax_amount, '11000.00');
    assertEquals(row.tax_template, 'Indonesia PPN 11% - RIS');
    assertEquals(row.tax_treatment, 'inclusive', 'amount is the ERP grand_total, which includes taxes by definition');
  },
...
Deno.test({
  name: '#478 — a create with no ERP tax total still states tax_amount (0.00 = untaxed document, never unknown)',
  fn: async () => {
    const { client, calls } = makeFakeClient({ companies: { org_id: 'org-1' }, projects: { org_id: 'org-1' } });
    const writer = getReadModelWriter('revenue');
    await writer.upsert(
      { serviceClient: client as never, orgId: 'org-1' },
      { id: 'pmo-si-478-2', si_number: 'ACC-SINV-2026-00479', amount: '5000.00', erp_outstanding_amount: '5000.00', erp_docstatus: 0, erp_modified: '2026-08-19 10:00:00.000000' },
      { domain: 'revenue', operation: 'create', record: { id: 'pmo-si-478-2', projectId: 'proj-1', customerId: 'cust-1', erp_doc_kind: 'sales-invoice' } },
    );
    const row = (calls.find((c) => c.method === 'insert' && c.table === 'sales_invoices'))!.args[0] as Record<string, unknown>;
    assertEquals(row.tax_amount, '0.00', 'tax_amount is NOT NULL with no DB default — the create branch must always state it');
    assertEquals(row.tax_treatment, 'inclusive');
  },
});
...
Deno.test({
  name: '#478 — a status-sync UPDATE omits currency/tax_amount/tax_template when the canonical does not carry them (never null over a recorded fact)',
  fn: async () => {
    const { client, calls } = makeFakeClient({ companies: { org_id: 'org-1' }, projects: { org_id: 'org-1' } });
    const writer = getReadModelWriter('revenue');
    await writer.upsert(
      { serviceClient: client as never, orgId: 'org-1' },
      { id: 'pmo-si-478-3', si_number: 'ACC-SINV-2026-00480', amount: '5000.00', erp_outstanding_amount: '0.00', erp_docstatus: 1, erp_modified: '2026-08-19 11:00:00.000000' },
      { domain: 'revenue', operation: 'transition', record: { id: 'pmo-si-478-3', erp_doc_kind: 'sales-invoice', externalRecordId: 'ACC-SINV-2026-00480', verb: 'submit' } },
    );
    const patch = (calls.find((c) => c.method === 'update' && c.table === 'sales_invoices'))!.args[0] as Record<string, unknown>;
    assert(!('currency' in patch), 'currency is NOT NULL — an absent value must be omitted, not written as null');
    assert(!('tax_amount' in patch), 'tax_amount is NOT NULL — an absent value must be omitted, not written as null');
    assert(!('tax_template' in patch), 'nulling a recorded template on a status tick would erase a fact ERP still holds');
    assertEquals(patch.tax_treatment, 'inclusive', 'the marker rides with amount, which this patch sets from grand_total');
  },
