// Task 1.8 (AC-EAC-010) — erpnext-onboard Vault credentials resolution
...
Deno.test({
  ...
  fn: async () => {
    const result = await resolveErpCredentialsFromVault('ref-1', async () => 'key-123:secret-456');
    assertEquals(result, { apiKey: 'key-123', apiSecret: 'secret-456' });
  },
});
